#!/usr/bin/env python3
"""Capture current Claude `/release-notes` and `/recap` UI without paid I/O.

The harness uses disposable home/config/workspace roots. Recap scenarios are
seeded directly into disposable Claude journals and all Anthropic message I/O
is forced to a deterministic HTTP fixture on 127.0.0.1 with a dummy key. No
normal conversation prompt or commercial-provider request is made.
"""
...
RECAP_TEXT = (
    "The local command audit is preserving the no-paid-provider boundary; "
    "next compare the native terminal evidence."
)
    ...
    def wait_for(self, *needles: str, timeout: float = 30) -> str:
        ...
        while time.monotonic() < deadline:
            ...
            if self.process.poll() is not None:
                raise RuntimeError(
                    f"Claude exited with {self.process.returncode} waiting for "
                    f"{needles}:\n{current}"
                )
...
def run(output: Path, *, columns: int, rows: int, timeout: float) -> dict[str, object]:
            ...
            try:
                event(
                    "message_start",
                    {
                        "type": "message_start",
                        "message": {
                            "id": f"msg_local_recap_{scenario}",
                            "type": "message",
                            "role": "assistant",
                ...
                event(
                    "content_block_delta",
                    {
                        ...
                        "index": 0,
                        "delta": {"type": "text_delta", "text": RECAP_TEXT},
                    },
                )
    ...
    result: dict[str, object] = {
        "status": "failed",
        "scope": "Claude Code release-notes and recap UI with local fixtures",
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="claude-session-info-reference-") as folder:
            root = Path(folder)
            ...
            changelog_cache = config / "cache"
            changelog_cache.mkdir()
            (changelog_cache / "changelog.md").write_text(
                "# Changelog\n\n"
                "## 2.1.268\n\n"
                "- SYNTHETIC RELEASE ALPHA: local picker fixture.\n"
                "- SYNTHETIC RELEASE BETA: no external fetch required.\n\n"
                "## 2.1.267\n\n"
                "- SYNTHETIC RELEASE GAMMA: previous local version.\n"
            )
            ...
            surfaces = output / "local-surfaces"
            surfaces.mkdir()
            ...
            terminal = ClaudePty(
                ...
                environment,
                workspace,
                [*common, "--session-id", surface_id, "--name", "session-info-surfaces"],
                columns=columns,
                rows=rows,
            )
            try:
                terminal.ready()
                terminal.capture(surfaces, "00-empty")
                terminal.send(b"/recap\r")
                empty_recap = terminal.wait_for("Nothing to recap yet", timeout=20)
                assert "send a message first" in empty_recap, empty_recap
                terminal.capture(surfaces, "01-recap-empty")

                terminal.send(b"/release-notes\r")
                picker = terminal.wait_for("Release notes", "Select", timeout=20)
                terminal.capture(surfaces, "02-release-notes-picker")
                assert "release" in picker.lower(), picker
                terminal.send(b"\x1b[B")
                terminal.capture(surfaces, "03-release-notes-keyboard-selection")
                terminal.send(b"\r")
                terminal.wait_for("SYNTHETIC RELEASE", timeout=20)
                terminal.capture(surfaces, "04-release-notes-selected")
                terminal.send(b"\x1b")
                terminal.read(0.5)
                terminal.capture(surfaces, "05-release-notes-after-selection")
            finally:
                (surfaces / "terminal.ansi").write_bytes(terminal.transcript)
                terminal.close()

            for scenario in ("complete", "failure", "cancel"):
                scenario_output = output / f"recap-{scenario}"
                ...
                bootstrap = ClaudePty(
                    ...
                    environment,
                    workspace,
                    [*common, "--session-id", session_id, "--name", f"recap-{scenario}"],
                    columns=columns,
                    rows=rows,
                )
                try:
                ...
                try:
                    ...
                    terminal.send(b"/recap\r")
                    terminal.wait_event(entered, "localhost recap request", timeout)
                    if scenario == "complete":
                        working = terminal.capture(scenario_output, "01-working")
                        ui_evidence["working_state_captured"] = "recap" in working.lower()
                        release.set()
                        terminal.wait_event(settled, "localhost recap completion", timeout)
                        completed = terminal.wait_for(RECAP_TEXT, timeout=20)
                        assert "".join(RECAP_TEXT.split()) in "".join(completed.split()), completed
                        terminal.capture(scenario_output, "02-completed")
                        ui_evidence["completed_one_line_recap"] = True
                    elif scenario == "failure":
                        terminal.wait_event(settled, "localhost recap failure", timeout)
                        failed = terminal.wait_for(
                            "Couldn't generate a recap", "LOCAL_RECAP_FAILURE", timeout=20
                        )
                        terminal.capture(scenario_output, "01-failed")
                        ui_evidence["explicit_failure_receipt"] = (
                            "Couldn't generate a recap" in failed
                            or "LOCAL_RECAP_FAILURE" in failed
                        )
                    else:
                        terminal.capture(scenario_output, "01-working")
                        terminal.send(b"\x1b")
                        terminal.read(0.8)
                        release.set()
                        terminal.wait_event(settled, "cancelled recap request settlement", timeout)
                        terminal.read(1.0)
                        cancelled = terminal.visible()
                        terminal.capture(scenario_output, "02-cancelled")
                        assert "Flummoxing" not in cancelled, cancelled
                        assert "/recap" in cancelled, cancelled
                ...
                assert scenario_requests, (scenario, requests)
                assert all(
                ...
                result["scenarios"][scenario] = {
                    ...
                    "request_sha256": scenario_requests[0]["request_sha256"],
