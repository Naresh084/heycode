//! Validated Grok process and catalog configuration.
...
/// Capture the minimum non-credential environment used by installed Grok.
///
/// Home/XDG paths permit Grok to use its own account/config stores. API
...
/// name-sorted.
#[must_use]
pub fn grok_environment_snapshot() -> Vec<(OsString, OsString)> {
    grok_environment_from(std::env::vars_os())
}

fn grok_environment_from<I>(values: I) -> Vec<(OsString, OsString)>
where
...
/// Operation-time Grok discovery and explicit child-environment policy.
#[derive(Clone)]
...
impl GrokRuntimeConfig {
    /// Construct the default `grok` discovery configuration.
    ///
    /// # Errors
    ...
    pub fn new(catalog_workspace: impl Into<PathBuf>) -> Result<Self, RuntimeContractError> {
        ...
        if !catalog_workspace.is_absolute() || contains_nul(catalog_workspace.as_os_str()) {
            return Err(invalid(
                "Grok catalog workspace",
                "an absolute NUL-free path",
            ));
        }
        Ok(Self {
            program: OsString::from("grok"),
            catalog_workspace,
            environment: grok_environment_snapshot(),
        })
    }
    ...
    pub fn with_program(
        ...
        if program.is_empty() || contains_nul(&program) || os_len(&program) > 4096 {
            return Err(invalid("Grok program", "1..=4096 NUL-free bytes"));
        }
...
impl Debug for GrokRuntimeConfig {
    fn fmt(&self, formatter: &mut Formatter<'_>) -> std::fmt::Result {
        formatter
            .debug_struct("GrokRuntimeConfig")
...
mod tests {
    ...
    fn installed_environment_is_sorted_and_credential_blind() {
        let environment = grok_environment_from([
