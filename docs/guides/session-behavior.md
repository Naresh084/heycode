...
`scripts/cache_quality_benchmark.py` is an opt-in paid native-CLI benchmark (`--execute` is required). It uses disposable fixture workspaces, a fixed model, a request cap, and an observed-cost cap. Its baseline relay removes only message cache markers. It records original-request hashes, transmitted fixture bodies, raw provider usage/costs, parent and child session evidence, and scored no-tool/read/five-agent tasks. It never stores authorization headers. Cached-input share and cost per correct task are reported separately; this small experiment cannot establish a universal caching or quality guarantee.
